...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.5):  # 调整dropout
        super().__init__()
...
class FCUDown(nn.Module):
    def __init__(self, inplanes, outplanes, dw_stride=2, act_layer=nn.GELU,
        ...
        self.dw_stride = max(1, dw_stride)  # 确保 dw_stride 至少为1
...
data = pd.read_csv(r'/home/data.csv', low_memory=False)
...
chunk_size = 10
dataset = ChunkedDataset(r'/home/data.csv', chunk_size, scaler)
...
# 找点
ensemble_model.eval()
feature_importance = np.zeros(input_data.shape[1])  # 初始化特征重要性数组，大小为输入数据的特征维度

print("\n开始计算特征重要性...")
for inputs, targets in tqdm(val_dataloader, desc="Computing Feature Importance"):
    inputs, targets = inputs.to(device), targets.to(device)
    inputs = inputs.unsqueeze(1).unsqueeze(2)  # 调整输入形状为 (batch_size, 1, 1, features)
    inputs.requires_grad = True  # 启用梯度追踪

    # 使用 detach() 去除梯度追踪后再转化为 NumPy 格式
    x_rf = inputs.view(inputs.size(0), -1).detach().cpu().numpy()  # 使用 detach() 以移除梯度追踪

    # 调用集成模型，传入 inputs 和 x_rf
    outputs = ensemble_model(inputs, x_rf)  # 注意这里传入了两个参数：inputs 和 x_rf
    ...
    feature_importance += np.sum(grads, axis=0)  # 累加每个特征的梯度值

# 计算平均特征重要性
feature_importance /= len(val_dataloader.dataset)

# 输出前 10 个权重最大的特征列名称
feature_names = data.columns[6:]  # 从原始数据中获取特征名
...
for i, feature in enumerate(top10_features):
    print(f"第{i+1}个特征：{feature} (重要性得分: {feature_importance[top10_indices[i]]:.6f})")
